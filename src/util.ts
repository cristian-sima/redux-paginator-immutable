...
const manageEntity = (normalizrEntity: NormalizrEntity) => (item: any, state: any, keyID: string): ManageDataEntity => {
  ...
  return old;
      ...
      manipulateItems = (sortKey: string = "ID") => createSelector(data => data, data => data.sortBy(current => -current.get(sortKey)));

// data item
const manageDataEntity = (normalizrEntity: NormalizrEntity) => (item: any, state: any, keyID: string): ManageDataEntity => {
  // prevent rewrite
  const old = state.get(String(item[keyID]));
  ...
  return old;
      ...
      manipulateDataItems = (sortKey: string = "ID") => createSelector(data => data, data => data.sortBy(current => -current.getIn(["Data", sortKey])));
