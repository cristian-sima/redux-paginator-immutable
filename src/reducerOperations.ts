...
const requestPage = (state: PagesState, action: RequestPageAction) => {
  ...
  receivePageItems = (state: ItemsState, action: ReceivePageAction) => {
    const {
      ...
      manage: any = typeof manageEntity === "function" ? manageEntity : (item) => Immutable.Map(item),
      newItems = payloadItems.reduce((previous, item) => previous.set(String(item[idKey]), manage(item, state, idKey)), Immutable.Map());
