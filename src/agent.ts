...
export const buildSuffix = (pageArgName: string, page: number, params: string) => {
  ...
  for (const key of Object.keys(parsedParams)) {
    if (parsedParams[key] === null) {
      ...
      finalParsedParams = { ...finalParsedParams,
  ...
  return startString + qs.stringify({ ...finalParsedParams,
    [pageArgName]: page
  }, {
    encode: false
  }).replace(startString, "");
...
export const fetchPage = ({
  ...
  const suffix = buildSuffix(pageArgName, page, token),
        url = endpoint + suffix;
  const promise: any = new Promise((resolve: any, reject: any) => agent.get(url).end((err: any, res: any) => err ? reject(err) : resolve(res)));
