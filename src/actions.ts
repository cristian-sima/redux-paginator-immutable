...
const fetchDataItemRequest = ({
  ...
  normalizeDataItem,
  id
}: FetchDataItem): Promise<any> => (new Promise((resolve, reject) => agent.get(`${dataItemURL}/${id}`).set("Accept", "application/json").end(normalizeDataItem(resolve, reject))) as Promise<any>);
