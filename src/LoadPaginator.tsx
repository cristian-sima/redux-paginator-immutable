...
const mapStateToProps = (state, {
  ...
  const currentPage = selectors.getCurrentView(list, token),
...
class LoadPaginator extends Component<LoadPaginatorPropTypes> {
  ...
  constructor(props: LoadPaginatorPropTypes) {
    ...
    this.handleLoadMoreClick = () => {
      ...
      const currentPage = paginator.get("currentPage"),
  ...
  UNSAFE_componentWillMount() {
    ...
    const isFetched = paginator.get("isFetched"),
  ...
  render() {
    ...
    const items = paginator.get("items"),
          hasProblems = paginator.get("hasProblems"),
          currentPage = paginator.get("currentPage"),
          isFetching = paginator.get("isFetching"),
    ...
    if (hasProblems && currentPage === 1) {
      return <LargeErrorMessage message={words.ThereWasAProblem} onRetry={this.handleLoadMoreClick} />;
    }

    return <React.Fragment>
        {React.cloneElement(this.props.children, this.props)}
        {showLoading ? <LoadingButton hasProblems={hasProblems} isFetching={isFetching} onLoadMoreClick={this.handleLoadMoreClick} /> : null}
      </React.Fragment>;
  }
